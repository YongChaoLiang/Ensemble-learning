...
def plot(final_scores1, final_scores2, final_scores3):
    ...
    for final_scores in [final_scores1, final_scores2, final_scores3]:
        plt.subplot(1, 3, k)
        ax = sns.barplot(x=final_scores.sort_values(strs[k-1])['Regressors'], y=final_scores.sort_values(strs[k-1])[strs[k-1]])
        # plt.ylim(0.1, 0.15)
...
print(final_score1)
final_score1.to_excel('MSE.xlsx', index=False)
final_score2.to_excel('MAE.xlsx', index=False)
final_score3.to_excel('R.xlsx', index=False)

x_train, x_test, y_train, y_test, samplein, sampleout, x_scaler, y_scaler = read_data()

# Blend models in order to make the final predictions more robust to overfitting
def blended_predictions(data):
    return ((0.25 * rf.predict(data)) +
            (0.1 * lgbm.predict(data)) +
            (0.25 * knn.predict(data)) +
...
print(y_test.shape)
print(blended_pre.shape)
pd.DataFrame(blended_pre).to_excel('blended_pre.xlsx')
pd.DataFrame(y_test).to_excel('blended_pre_ytest.xlsx')
...
pd.DataFrame(scores_pearson, index=[0]).to_excel('zhexian_Test_R.xlsx')
pd.DataFrame(train_scores_pearson, index=[0]).to_excel('zhexian_Train_R.xlsx')
pd.DataFrame(scores_mse, index=[0]).to_excel('zhexian_Test_mse.xlsx')
pd.DataFrame(train_scores_mse, index=[0]).to_excel('zhexian_Train_mse.xlsx')
pd.DataFrame(scores_mae, index=[0]).to_excel('zhexian_Test_mae.xlsx')
pd.DataFrame(train_scores_mae, index=[0]).to_excel('zhexian_Train_mae.xlsx')
...
# print(rftrain_pre.shape)
# print(train_pearsons.shape)
...
pd.DataFrame(rfy_train).to_excel('rfy_train.xlsx')
pd.DataFrame(rftrain_pre).to_excel('rftrain_pre.xlsx')
pd.DataFrame(rfy_test).to_excel('rfy_test.xlsx')
pd.DataFrame(rfy_pre).to_excel('rftest_pre.xlsx')
...
pd.DataFrame(ly_train).to_excel('ly_train.xlsx')
pd.DataFrame(lgbmtrain_pre).to_excel('lgbmtrain_pre.xlsx')
pd.DataFrame(ly_test).to_excel('ly_test.xlsx')
pd.DataFrame(lgbmy_pre).to_excel('lgbmtest_pre.xlsx')
...
pd.DataFrame(ky_train).to_excel('ky_train.xlsx')
pd.DataFrame(knntrain_pre).to_excel('knntrain_pre.xlsx')
pd.DataFrame(ky_test).to_excel('ky_test.xlsx')
pd.DataFrame(knny_pre).to_excel('knntest_pre.xlsx')
...
pd.DataFrame(sy_train).to_excel('sy_train.xlsx')
pd.DataFrame(stacktrain_pre).to_excel('stacktrain_pre.xlsx')
pd.DataFrame(sy_test).to_excel('sy_test.xlsx')
pd.DataFrame(stacky_pre).to_excel('stacktest_pre.xlsx')
# plt.savefig('scatter.svg', dpi=600, format='svg')
plt.show()
pd.DataFrame(train_pearsons).to_excel('train_pearsons.xlsx')
pd.DataFrame(train_mses).to_excel('train_mses.xlsx')
pd.DataFrame(train_maes).to_excel('train_maes.xlsx')
pd.DataFrame(test_pearsons).to_excel('test_pearsons.xlsx')
pd.DataFrame(test_mses).to_excel('test_mses.xlsx')
pd.DataFrame(test_maes).to_excel('test_maes.xlsx')
